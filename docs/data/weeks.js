window.SCIML_WEEKS = [
  {
    ...
    intuition: 'A PINN replaces an unknown field u with a differentiable neural approximation uθ and asks the optimizer to reduce violations of the PDE, initial conditions, and boundary conditions at sampled points.',
    equation: 'L(θ) = λr Lr + λIC LIC + λBC LBC + λd Ldata',
    papers: [
  ...
  },
  {
    ...
    intuition: 'Two distinct biases appear: networks often learn low-frequency structure first, and time-dependent PDE errors propagate forward even when the loss treats time slices too independently.',
    equation: 'u = Σk ûk ϕk,     Lcausal = Σi wi(θ) Li',
    papers: [
  ...
  },
  {
    ...
    intuition: 'Wave problems expose numerical design choices brutally: where collocation points are placed, how oscillations are represented, and whether the truncated domain reflects energy all change the optimization landscape.',
    equation: 'Rθ = utt,θ − c²Δuθ − s,     z ~ qθ(z),     BABC[u] ≈ 0',
    papers: [
  ...
  },
  {
    ...
    intuition: 'NeuSA represents the field in a spectral basis, turns known operators into analytical coefficient dynamics, and learns a correction that is then integrated through time as a neural ODE.',
    equation: 'uM(t,x)=Σk ak(t)ϕk(x),     ȧ = b,     ḃ = Fknown(a)+Fθ(a)',
    papers: [
